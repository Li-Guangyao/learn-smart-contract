...
*/
    ...
    function addReward(
        ...
        require(
            userMapping[userAddr].status == 1,
            "This address is not a DAO member or has been blacklisted"
        );

        require(addEXP >= 0, "The EXP added must be a positive number");

        require(
            bytes(badgeMapping[badgeID]).length != 0,
            "Badge doesn't exist"
        );
...
*/
    ...
    function SBTOf(address owner) public view returns (uint256) {
